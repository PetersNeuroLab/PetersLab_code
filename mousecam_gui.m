...
view_button_h(end+1) = uicontrol('Parent',gui_fig,'Style','togglebutton','FontSize',button_fontsize, ...
    'Units','normalized','Position',view_button_position,'BackgroundColor','w', ...
    'String','Record','Callback',{@cam_record,gui_fig});
...
disp('LISTEN FUNCTION NOT SET UP YET')

end

function cam_record(h,event_data,gui_fig)
% Record movie

% Get GUI data
gui_data = guidata(gui_fig);

switch event_data.Source.Value

    case true

        %%%%%%% TO DO: TEMPORARY
        local_data_path = 'C:\Users\peterslab\Documents\MATLAB\face_camera\mmmgui_test\';

        [cam_file,cam_path] = uiputfile(fullfile(local_data_path,'*.mj2'),'Save movie as...');
        ...
        % Set button red
        event_data.Source.BackgroundColor = 'r';
    ...
    case false
        ...
        % Set button white
        event_data.Source.BackgroundColor = 'w';
